####functions####
data_gen <- function(i,n_V_se,p,Sigma0,error_type,tau){
  source("main_reg model.R")
  set.seed(20250718+8*i)
  X <- mvrnorm(n, rep(0,p), Sigma0)
  if(error_type=="Gaussian"){
    error <- rnorm(n,0,1)
  }
  y <- sapply(1:3, function(i){
    X[(tau_all0[i]+1):(tau_all0[i+1]),] %*% beta_all[,i]
  })
  y <- as.matrix(unlist(y) + error, ncol=1)
  return(list(X=X,y=y))
}
...
main_ART_fun <- function(i, error_type,inter_type){
  ...
  if(s0==p){
    ...
    lambda0 = sqrt(log(p)/n)
  }
  beta_can <- glmnet(X,y,family = "gaussian",lambda = lambda0, intercept = FALSE)$beta
  score <- y - X %*% beta_can
  V <- abs(score)
...
main_nsp_fun <- function(i, error_type){
  ...
  intervals = create_seed_int(n,d=4)
  M = dim(intervals)[1]
  #nsp
