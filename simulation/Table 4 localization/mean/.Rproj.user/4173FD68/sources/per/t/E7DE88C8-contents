...
for (ee in 1:length(error_type_all)) {
  for (ii in 1:length(inter_type_all)) {
    ...
    COVERAGE_rate <- 1 - sum(COVERAGE==0)/ length(COVERAGE)
    ...
    print(paste0(error_type_all[ee],".",inter_type_all[ii]))
...
error_type_all <- c("Gaussian","t")
for (ee in 1:length(error_type_all)) {
  print(paste("NSP","error_type=",error_type_all[ee]))
  RES <-foreach(i=1:rep_num,.combine = "rbind") %dopar%
    main_nsp_fun(i, error_type_all[ee])
  RES_ind <- sapply(1:rep_num, function(i){
    is.null(RES[i,"interval_select_all"][[1]])
  })
  RES_ind <- which(RES_ind==F)
...
error_type_all <- c("Gaussian","t")
for (ee in 1:length(error_type_all)) {
  print(paste("RNSP","error_type=",error_type_all[ee]))
  RES <-foreach(i=1:rep_num,.combine = "rbind") %dopar%
    main_rnsp_fun(i, error_type_all[ee])
  RES_ind <- sapply(1:rep_num, function(i){
    is.null(RES[i,"interval_select_all"][[1]])
  })
  RES_ind <- which(RES_ind==F)
