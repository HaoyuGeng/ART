...
ART_cp <- function(V, intervals,Sn_Il_all, tau_est_all, n, lambda_alpha){
...
main_ART_cp_fun <- function(score_all, inter_type){
  
  library(MASS)
 
  n <- length(score_all)
  V <- rank(score_all, ties.method = "random")
  ...
  if(inter_type=="seed"){
    ...
    #compute threshold
    lambda_alpha <- 0.57568 #threshold_alpha_fun(intervals, n, 0.1)   
  ...
  ART_result <- ART_cp(V, intervals, Sn_Il_all, tau_est_all, n, lambda_alpha)
...
coverage_fun <- function(select_interval, tau_all0){
  ...
  coverage <- sapply(1:(dim(select_interval)[1]), function(i){
    sum( (select_interval[i,1] <= tau_all0) & (select_interval[i,2] >= tau_all0)) >= 1
  })
  
  if(sum(coverage)== dim(select_interval)[1]){
    is.coverage = 1
  }else{
    is.coverage = 0
  }
...
correct_interval_num_fun <- function(select_interval, tau_all0){
  tau_all0 = tau_all0[2:(length(tau_all0)-1)]
  correct_int <- sapply(1:dim(select_interval)[1], function(i){
    sum( (select_interval[i,1] <= tau_all0) & (select_interval[i,2] >= tau_all0)) >= 1
  })
...
length_correct_interval_fun <- function(select_interval, tau_all0){
  ...
  length_all <-sapply(1:dim(select_interval)[1], function(i){
    select_interval[i,2] - select_interval[i,1]
  })
